   ...
   for (/*use current i*/; i < sbIn->stmts_used; i++) {
      ...
      switch (st->tag) {
         ...
         case Ist_IMark:
            cia   = st->Ist.IMark.addr;
            isize = st->Ist.IMark.len;
            set_current_instruction_addr(&memviz_profiler, st->Ist.IMark.addr);
...
static Bool handle_gdb_monitor_command (ThreadId tid, const HChar *req) {
   ...
   switch (VG_(keyword_id) ("checkpoint", 
      ...
      case  0: /* checkpoint */
         ...
         checkpoint = VG_(strtok_r)(0, " ", &ssaveptr);
         record_checkpoint(&memviz_profiler, checkpoint, tid);
         return True;
...
static
Bool handle_client_request(ThreadId tid, UWord *args, UWord *ret)
{
   switch(args[0]) {
...
{
   VG_(details_name)            ("MemViz");
   VG_(details_version)         (NULL);
   VG_(details_description)     ("CacheGrind modified to record memory traffic");
...
static void cg_post_clo_init(void)
{
   ...
   if (min_line_size < largest_load_or_store_size) {
      ...
      VG_(umsg)("MemViz: cannot continue: the minimum line size (%d)\n",
                (Int)min_line_size);
